 ...
 */
export const appStatusInfo: Writable<AppStatusInfo> = writable({
  id: " ",
  url: "",
  pages: 0,
});
 ...
 */
export const setChat = (id: string, url: string, pages: number): boolean => {
  try {
    appStatusInfo.set({ id, url, pages });
    return true;
  } catch (error) {
    console.error("Error setting chat info:", error);
    return false;
  }
};
